...
export default function AdminRegistrationsPage() {
  ...
  useEffect(() => {
    if (loggedIn) {
      // Auth just became true — fetch the registrations list. setState
      // happens after the async fetch resolves, not synchronously in the
      // effect body.
  ...
  const getStatus = (r: Registration): "pending" | "accepted" | "declined" =>
    r.status === "accepted" ? "accepted" : r.status === "declined" ? "declined" : "pending";
  const getPayment = (r: Registration): "paid" | "pending" =>
  ...
  const filteredRegs = useMemo(() => {
    ...
    return regs.filter((r) => {
      ...
      if (paymentFilter !== "all" && getPayment(r) !== paymentFilter) return false;
      if (q) {
        const name = `${r.firstName} ${r.lastName}`.toLowerCase();
        const car = `${r.brand} ${r.model}`.toLowerCase();
        if (!name.includes(q) && !car.includes(q)) return false;
      }
      return true;
    });
    ...
    <div className="grid gap-8">
      {[0, 1, 2].map((i) => (
        <div key={i} className="bg-[#111]/90 border border-gray-700 rounded-sm overflow-hidden">
          <div className="flex flex-wrap items-center gap-6 px-4 py-4 sm:px-6 bg-white/5 border-b border-gray-700">
          ...
          <div className="p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
            <div className="lg:col-span-6 xl:col-span-5 grid grid-cols-2 gap-2 sm:gap-3">
